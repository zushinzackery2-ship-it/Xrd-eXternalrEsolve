...
inline std::vector<uptr> CollectClassObjectsForResolve(
    const IMemoryAccessor& mem,
    const UEOffsets& off,
    i32 limit = 1024)
{
    ...
    i32 total = GetObjectCount(mem, off);
    for (i32 i = 0; i < total && static_cast<i32>(classObjects.size()) < limit; ++i)
    {
        uptr obj = ReadObjectAt(mem, off, i);
        if (!IsCanonicalUserPtr(obj))
        {
...
inline uptr FindClassObjectByShortNameForResolve(
    const IMemoryAccessor& mem,
    const UEOffsets& off,
    const std::vector<uptr>& classObjects,
    const std::string& shortName)
{
    for (uptr obj : classObjects)
    {
        FName fn{};
        if (!ReadValue(mem, obj + off.UObject_Name, fn))
        {
    ...
    UEOffsets& off)
{
    ...
    if (!classObj || !structObj || !fieldObj)
    {
        std::vector<uptr> classObjects = CollectClassObjectsForResolve(mem, off);
        if (!classObj)
        {
            classObj = FindClassObjectByShortNameForResolve(mem, off, classObjects, "Class");
        }
        if (!structObj)
        {
            structObj = FindClassObjectByShortNameForResolve(mem, off, classObjects, "Struct");
            if (!structObj)
            {
                structObj = FindClassObjectByShortNameForResolve(mem, off, classObjects, "struct");
            }
        ...
        if (!fieldObj)
        {
            fieldObj = FindClassObjectByShortNameForResolve(mem, off, classObjects, "Field");
        }
    }
