...
inline bool AutoInitSharedMem(const wchar_t* processName = nullptr)
{
    ...
    u64 moduleSize = 0;
    u64 moduleBase = shmemMem->GetMainModule(&moduleSize);
    if (!moduleBase)
    {
    ...
    ctx.mainModule.size = moduleSize > 0xFFFFFFFFull
        ? 0xFFFFFFFFu
...
// ─── 驱动模式初始化：统一使用 SharedMemoryAccessor（DAT 握手 + 共享内存） ───
// 旧的 DriverMemoryAccessor (IOCTL) 已废弃，AutoInitDriver 现在等同于 AutoInitSharedMem。
