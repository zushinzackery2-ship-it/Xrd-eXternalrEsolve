...
// 参数结构体成员类型（默认使用原始类型，仅保留 const pointer 语义）
inline std::string BuildParamMemberType(const FunctionParam& p)
{
    std::string type = p.typeName;
    const bool isPointer = type.find('*') != std::string::npos;
    const bool hasConstPrefix = type.rfind("const ", 0) == 0;
    if (p.isConstParam && isPointer && !hasConstPrefix)
    {
        type = "const " + type;
    }
    return type;
    ...
    const FunctionInfo& func)
{
    ...
    for (auto& p : func.params)
    {
        ...
        out += MakeMemberString(
            BuildParamMemberType(p), p.name, comment);
...
// 参数结构体断言宏（仅校验 sizeof，避免不稳定的 align 误报）
inline std::string GenerateParamAssertionMacro(
    const std::string& assertName,
    const std::string& typeName,
    i32 size)
{
