...
// 从 SkeletalMeshComponent 获取 SkeletalMesh 指针（带缓存）
// meshComponent → skelMeshPtr 运行时不变，缓存后避免每帧 2 次 RPM
namespace detail
{
    inline std::unordered_map<uptr, uptr>& SkelMeshCache()
    {
        static std::unordered_map<uptr, uptr> c;
        return c;
    }
    inline std::mutex& SkelMeshCacheMutex()
    {
        static std::mutex m;
        return m;
    }
...
inline uptr GetSkeletalMesh(uptr meshComponent)
{
    ...
    // 快速路径：缓存命中
    {
        ...
        auto it = detail::SkelMeshCache().find(meshComponent);
        if (it != detail::SkelMeshCache().end())
        {
            return it->second;
        }
    }
    ...
    {
        std::lock_guard<std::mutex> lock(detail::SkelMeshCacheMutex());
        detail::SkelMeshCache()[meshComponent] = skelMesh;
    }
    return skelMesh;
}
