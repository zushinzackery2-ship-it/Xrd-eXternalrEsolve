    ...
    std::string& out)
{
...

    {
        ...
        auto it = detail::NameCache().find(compIdx);
        if (it != detail::NameCache().end())
        {
            out = it->second;
            return !out.empty();
        }
    ...
    if (off.bUseNamePool)
    {
        ok = ResolveName_NamePool(mem, off.GNames, compIdx, resolved);
    }
    else
    {
        ok = ResolveName_Array(mem, off.GNames, compIdx, resolved);
    }
    ...
    {
        std::unique_lock<std::shared_mutex> wlock(detail::NameCacheMutex());
        auto [it, inserted] = detail::NameCache().try_emplace(compIdx, resolved);
        out = it->second;
    }
...
inline std::string GetNameFromFName(i32 compIdx, i32 number = 0)
{
    ...
    std::string name;
    bool ok = ResolveNameCached(Mem(), Off(), compIdx, name);
