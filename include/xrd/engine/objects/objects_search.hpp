...
inline i32 GetPropertyOffsetByName(uptr cls, const std::string& propName)
{
...

    {
        ...
        auto it = detail::PropOffsetCache().find(key);
        if (it != detail::PropOffsetCache().end())
        {
            return it->second;
        }
    }
    ...
    {
        std::unique_lock<std::shared_mutex> wlock(detail::PropOffsetCacheMutex());
        auto [it, inserted] = detail::PropOffsetCache().try_emplace(std::move(key), result);
        return it->second;
    }
}
