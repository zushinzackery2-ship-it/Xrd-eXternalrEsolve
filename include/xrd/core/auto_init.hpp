...
#include "../engine/physx/physx_pe.hpp"
...
// ─── 公共扫描逻辑（AutoInit / AutoInitDriver / AutoInitSharedMem 共用） ───
...
inline bool DoCommonScanAndDiscover()
{
    ...
    // PhysX 全局指针动态定位（通过 PE 导出表解析 PxGetPhysics）
    if (ctx.off.PhysXGlobalPtr == 0)
    {
        ...
        {
            std::cerr << "[xrd] PhysX 未找到（非致命，碰撞绘制不可用）\n";
        }
    }
...
// 初始化后所有 Read 走共享内存 + adaptive spinning，零 IOCTL 开销
